...
main(int argc, char **argv)
{
    ...
    banned_ips = rte_hash_create(&ip_hash_params);

    /* Leo el fichero de IPs */
    if (argv != NULL)
    {
        FILE *ip_file = fopen("banned", "r");       //LEER DESDE ARGV
        ...
        if (ip_file != NULL)
        {
            while (!feof(ip_file))
            {
                fgets(buffer, MAX_STRING_LENGTH, ip_file);

                if (1)
                {
                    printf("IP leída: %s\n", buffer);
                    inet_pton(AF_INET, buffer, &bin_ip);
                    printf("IP en binario: %u\n", bin_ip);
                    
                    int ret = rte_hash_add_key(banned_ips, &bin_ip);
                    if (ret < 0) {
